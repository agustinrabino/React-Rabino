    ...
    return(
        <Stack direction="horizontal" gap={2} className="d-flex align-items-center">
            ...
            <div className="me-auto d-flex flex-column justify-content-center">
                ...
                <div style={{fontSize:"10px", fontStyle:"italic"}}>{`unit price: ${FormatCurrency(item.price)}`}</div>
                <div style={{fontSize:"10px", fontStyle:"italic"}} className="d-flex  align-items-center" >
                    <span>{`quantity: `} </span>
                    <MyButton  onClick={()=>decreaseQuantity(id)} buttonStyle="countShoppingCart" className="btn">-</MyButton>
                    <span>{` ${quantity} `}</span>
                    <MyButton  onClick={()=>increaseQuantity(id)} buttonStyle="countShoppingCart" className="btn">+</MyButton>
                </div>
            </div>
